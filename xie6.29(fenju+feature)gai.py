...
overall_feature = csv_input[['word_count', 'sent_count', 'lemma_count', 'spell_err_count', 'noun_count', 'adj_count', 'verb_count', 'adv_count']]
...
text_train, text_test, label_train, label_test, overall_train, overall_test = train_test_split(essays_pad, Score, overall_feature, test_size=0.3, random_state=1)

model = get_model()
model.compile(loss='mean_squared_error', optimizer=Adam(lr=0.001, decay=1e-6), metrics=['mse'])
model.fit([text_train, overall_train], label_train, batch_size=32, epochs=50)
score_predict = model.predict([text_test, overall_test])
score_predict = np.around(score_predict)
result = cohen_kappa_score(label_test.values, score_predict, weights='quadratic')
print("Kappa Score: {}".format(result))
