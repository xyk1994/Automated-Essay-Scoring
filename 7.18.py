...
Score = np.array(csv_input['domain1_score'])
Score = (Score - 2) / 10
...
def get_model():
    ...
    lstm1 = LSTM(64, dropout=0.5, recurrent_dropout=0.5, return_sequences=True)(embedding)  # (?, ?, 64)
    ...
    lstm2 = LSTM(32, return_sequences=False)(sent_feature_concat_layer)
...
for train, test in cv.split(essays_pad):
    ...
    label_train, label_test = Score[train], Score[test]
    ...
    model.fit([text_train, overall_train, sent_train], label_train, batch_size=64, epochs=25, verbose=2)
    ...
    Label_test = (label_test * 10) + 2

    score_predict = np.around(score_predict)

    result = cohen_kappa_score(Label_test, score_predict, weights='quadratic')
...
score_predict_list = pd.DataFrame(score_predict_list)
score_predict_list.to_csv('data/xie7.22/predict_score.tsv', sep='\t')

label = pd.DataFrame(label)
label.to_csv('data/xie7.22/label.tsv', sep='\t')
