...
def get_model():
    ...
    lstm = LSTM(64, dropout=0.1, recurrent_dropout=0.1, return_sequences=True)(embedding)  # (?, ?, 64)

    a = [i for i in range(85, 4472, 86)]
    sent_layer = [Lambda(lambda t: t[:, i, :])(lstm) for i in a]
    sent_output = Lambda(stack)(sent_layer)
    #print(sent_output)
    #flatten_layer = Flatten()(sent_output)
    lstm2 = LSTM(32)(sent_output)
...
for train, test in cv.split(essays_pad):
    ...
    model.fit(text_train, label_train, batch_size=64, epochs=50)
