...
csv_input = csv_input[["essay", "rater1_domain1", "word_length", "comma_num", "apo_num", "end_num", "unk_num"]]
uni = max(csv_input.loc[:, "rater1_domain1"].values) + 1

print("AES with 5 Feature")
# print(csv_input)
# l = list(range(len(csv_input)- 1))F
# random.shuffle(l)
...
data_x, data_t, data_f1, data_f2, data_f3, data_f4, data_f5 = [], [], [], [], [], [], []  # x = text, t = label, f = feature
train_x, train_t, train_f1 ,train_f2, train_f3, train_f4, train_f5 = [], [], [], [], [], [], []


for d in range(N):
    data_x.append(data.ix[d, 0])  # 文書
    data_t.append(data.ix[d, 1])  # ラベル
    data_f1.append(data.ix[d, 2])  # 特徴量
    data_f2.append(data.ix[d, 3])
    data_f3.append(data.ix[d, 4])
    data_f4.append(data.ix[d, 5])
    data_f5.append(data.ix[d, 6])


for d in range(N):
    train_x.append(data.ix[d, 0])  # 文書
    train_t.append(data.ix[d, 1])  # ラベル
    train_f1.append(data.ix[d, 2])  # 特徴量
    train_f2.append(data.ix[d, 3])
    train_f3.append(data.ix[d, 4])
    train_f4.append(data.ix[d, 5])
    train_f5.append(data.ix[d, 6])
...
data_f1 = np.array(data_f1, dtype="int32")
train_f1 = np.array(train_f1, dtype="int32")
data_f2 = np.array(data_f2, dtype="int32")
train_f2 = np.array(train_f2, dtype="int32")
data_f3 = np.array(data_f3, dtype="int32")
train_f3 = np.array(train_f3, dtype="int32")
data_f4 = np.array(data_f4, dtype="int32")
train_f4 = np.array(train_f4, dtype="int32")
data_f5 = np.array(data_f5, dtype="int32")
train_f5 = np.array(train_f5, dtype="int32")


dataset = []
for x, t, f1, f2, f3, f4, f5 in zip(data_x_vec, data_t, data_f1, data_f2, data_f3, data_f4, data_f5):
    dataset.append((x, t, f1, f2, f3, f4, f5))
    ...
    for i in range(0, len(index), BATCH_SIZE):
        ...
        for n in range(BATCH_SIZE):
            ...
            if i + n >= (len(index)):
                ...
                feature1 = index[i + n - len(index), 2]
                feature2 = index[i + n - len(index), 3]
                feature3 = index[i + n - len(index), 4]
                feature4 = index[i + n - len(index), 5]
                feature5 = index[i + n - len(index), 6]
                feature = [feature1, feature2, feature3, feature4, feature5]
            else:
                text = index[i + n, 0]
                label = index[i + n, 1]
                feature1 = index[i + n, 2]
                feature2 = index[i + n, 3]
                feature3 = index[i + n, 4]
                feature4 = index[i + n, 5]
                feature5 = index[i + n, 6]
                feature = [feature1, feature2, feature3, feature4, feature5]
            Text.append(text)
            Label.append(label)
