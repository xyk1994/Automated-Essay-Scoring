...
for group_number in range(5):
    ...
    test_x_vec = to_word_idx_test(test_x, vocab_dict)
    ...
    # データセット
    data_x_vec = np.array(data_x_vec, dtype=np.int32)
    test_x_vec = np.array(test_x_vec, dtype=np.int32)
    data_t = np.array(data_t, dtype=np.float32)
    test_t = np.array(test_t, dtype=np.float32)
    data_f = np.array(data_f, dtype=np.float32)
    test_f = np.array(test_f, dtype=np.float32)
    # dataset = []
...
name2 = ["test_text"]
Text = pd.DataFrame(columns=name2, data=Text)
