...
def get_model():
    ...
    lstm1 = LSTM(64, return_sequences=True)(embedding)  # (?, ?, 64)
...
for train, test in cv.split(essays_pad):
    ...
    model.fit([text_train, sent_train], label_train, batch_size=64, epochs=50)
...
print("Average Kappa score after a 5-fold cross validation: ", np.around(np.array(results).mean(), decimals=5))
