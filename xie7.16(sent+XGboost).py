...
overall_feature = np.sqrt(np.array(overall_feature))
num_feature = 9
...
sentence_feature = np.array(pad_sent_feature)
sentence_feature = np.array([mat.T for mat in sentence_feature])
...
def get_model():
    ...
    # sentence_feature_input = Input(shape=(max_sent_length, 2,), name='sentence_feature_input')
    # print(text_input) # Tensor("text_input:0", shape=(?, 4472), dtype=float32)
    # print(overall_feature_input)
    # print(sentence_feature_input)
    ...
    lstm1 = LSTM(64, dropout=0.1, recurrent_dropout=0.1, return_sequences=True)(embedding)  # (?, ?, 64)
    ...
    lstm2 = LSTM(32, return_sequences=False)(sent_output)
    ...
    # dense1 = Dense(32, activation='sigmoid')(concat_layer)
    #dense1 = Dense(32, activation='sigmoid')(lstm2)
    #dense2 = Dense(16, activation='relu')(dense1)
    output = Dense(1, activation='relu')(lstm2)
...
for train, test in cv.split(essays_pad):
    ...
    # sent_train, sent_test = sentence_feature[train], sentence_feature[test]

    model = get_model()
    model.compile(loss='mean_squared_error', optimizer='adam', metrics=['mae'])

    model.fit(text_train, label_train, batch_size=32, epochs=50)

    score_predict = model.predict(text_test)
    score_predict_list.append(score_predict)
...
print("Average Kappa score after a 4-fold cross validation: ", np.around(np.array(results).mean(), decimals=4))

Score_predict_list = []
for list in score_predict_list:
    for score in list:
        Score_predict_list.append(score)
Score_predict_list = [score[0] for score in Score_predict_list]
Label = []
for list in label:
    for score in list:
        Label.append(score)
...
print(len(Score_predict_list))
print("---------------------------------------")
print(Label)
print(len(Label))
print("---------------------------------------")

Score_train, Score_test, Label_train, Label_test, overall_train, overall_test = train_test_split(Score_predict_list, Label, overall_feature, test_size=0.3, random_state=42)
Score_train = np.array(Score_train).reshape(1, -1)
Score_test = np.array(Score_test).reshape(1, -1)
overall_train = np.array(overall_train).reshape(1, -1)
overall_test = np.array(overall_test).reshape(1, -1)


train_set_xgb_input = np.concatenate((Score_train, overall_train), axis=1)
test_set_xgb_input = np.concatenate((Score_test, overall_test), axis=1)

model_xgb = XGBRegressor(learning_rate=0.02, n_estimators=1000, max_depth=10, n_jobs=-1)
model_xgb.fit(train_set_xgb_input, Label_train)

xgb_test_pred = model_xgb.predict(test_set_xgb_input)
QWK_test_result_xgb = cohen_kappa_score(xgb_test_pred, Label_test, weights='quadratic')
print("Total combined model for QWK on dev set: {:.04}".format(QWK_test_result_xgb))
